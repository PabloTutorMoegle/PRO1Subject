...
/* Pre: tree és un arbre binari no buit, strs és un vector de strings que conté paraules clau a cercar. 
	depth indica la profunditat actual de la recursió i maxDepth és la profunditat màxima actualitzada de manera recursiva */
/* Post: retorna el valor del node més profund que és comú a totes les paraules clau en strs i actualitza maxDepth amb la profunditat d'aquest node */
string findDeepestCommonNodeRec(const BinaryTree<T>& tree, const vector<string>& strs, int depth, int& maxDepth) {
  if (tree.isEmpty()) {
    return "";
  }

  string nodeValue = tree.getRoot(); 
  string deepestNode = "";
  bool found = true;
  ...
  if (found && depth > maxDepth) {
    maxDepth = depth;
    deepestNode = nodeValue;
  }

  string left = findDeepestCommonNodeRec(tree.getLeft(), strs, depth + 1, maxDepth);
  string right = findDeepestCommonNodeRec(tree.getRight(), strs, depth + 1, maxDepth);
...
/* Pre: tree és un arbre binari no buit, strs és un vector de strings que conté paraules clau a cercar. 
	depth indica la profunditat actual de la recursió i maxDepth és la profunditat màxima actualitzada de manera recursiva */
/* Post: retorna el valor del node més profund que és comú a totes les paraules clau en strs i actualitza maxDepth amb la profunditat d'aquest node */
  string findDeepestCommonNode(const vector<string>& strs, BinaryTree<string> &Tree) {
    int maxDepth = -1;
    return findDeepestCommonNodeRec(Tree, strs, 0, maxDepth);
  }
  ...
  //********************************************************* 
 //Pre: L'objecte actual apunta a un arbre binari vàlid.
 //Post: Retorna true si l'element amb el valor 'value' es troba a l'arbre, fals altrament.
  bool search(const T &value) const {
    ...
    return left->search(value) || right->search(value);
  }
